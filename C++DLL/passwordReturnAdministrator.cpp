#include "passwordReturnAdministrator.h"

//connect to MySQL databaseand return administrator's password
const char* returnPasswordAdministrator(const char* s)
{
	MYSQL mysql;
	MYSQL* ptr = &mysql;
	MYSQL_RES* res;
	...
	{
		std::string option = "select * from idpwadministrator where id =";
		option.append("'");
		option.append(s);
		option.append("'");
